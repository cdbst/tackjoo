...
function register(){
    ...
    ipcMain.on('save-proxy-info', (event, data) => {
        (async()=>{
            try{
                const proxy_info = data.payload.proxy_info;
                
                await UserFileManager.write(USER_FILE_PATH.PROXY_INFO, proxy_info);
    ...
    ipcMain.on('load-proxy-info', (event, data) => {
        (async()=>{
            try{
                ...
                event.reply('load-proxy-info-reply' + data.id, {err : undefined, data : proxy_info_data});
            }catch(err){
