...
module.exports.get_returnable_info_list_from_product_page = ($, account_email) => {
    ...
    $('.order-list').each((_idx, order_info_element)=>{
        ...
        try{
            ...
            if(el_span_date.length === 0) throw new Error('cannot found order_date');
            const el_date_text = get_specific_child_text_nodes(el_span_date[0]);
            if(el_date_text.length === 0) throw new Error('cannot found order_date - 2');
            ...
            if(el_span_order_price.length === 0) throw new Error('cannot found order_price');
            const el_span_order_price_child_text_list = get_specific_child_text_nodes(el_span_order_price[0]);
            if(el_span_order_price_child_text_list.length === 0) throw new Error('cannot found order_price - 2');
            ...
            if(el_span_order_number.length === 0) throw new Error('cannot found order_number');
            const el_span_order_child_text_list = get_specific_child_text_nodes(el_span_order_number[0]);
            if(el_span_order_child_text_list.length === 0) throw new Error('cannot found order_number - 2');
            ...
            if(el_input_order_id.length === 0) throw new Error('cannot found order_id');
            ...
            if(el_input_order_item_id.length === 0) throw new Error('cannot found order_item_id');
            ...
            if(el_input_returnable_quantity.length === 0) throw new Error('cannot found order returnable quantity');
            ...
            if(returnable_quantity !== 1) throw new Error('cannot support quantity over than one'); // 반품 수량이 한 개인 것만 지원한다.

            //# parse product_name
            const el_span_product_name = get_specific_tag_nodes(order_info_element, [], ['tit']);
            if(el_span_product_name.length === 0) throw new Error('cannot found product_name');
            const el_product_name_text = get_specific_child_text_nodes(el_span_product_name[0]);
            if(el_product_name_text.length === 0) throw new Error('cannot found product_name - 2');
            ...
            if(el_span_opts.length < 2) throw new Error('cannot found product_option and product_model_id');

            const el_product_model_id_text = get_specific_child_text_nodes(el_span_opts[0]);
            if(el_product_model_id_text.length === 0) throw new Error('cannot found model id');
            ...
            if(el_product_option_text.length === 0) throw new Error('cannot found product option');
            ...
            if(el_img_product_img.length === 0) throw new Error('cannot found product_img_url');
